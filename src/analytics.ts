...
function clientId() {
  const key = "netreceipt.analytics.client-id";
...
function initializeAnalytics() {
  if (!measurementId || !analyticsEnabled || initialized) return;

  window.dataLayer = window.dataLayer ?? [];
  // gtag.js expects the function's Arguments object rather than a regular array.
